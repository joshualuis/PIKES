...
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">Products /</span> Edit
</h4>

<div class="card mb-4">
   <form class="card-body" method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
      @method('PATCH')
      @csrf
   ...
   <div class="row g-3">
    ...
    <div class="row g-3">
      <div class="col-md-6">
        ...
        <input type="text" id="name" name="name" class="form-control" placeholder="Name" required value="{{old('name', $product->name) }}"/>
        @if ($errors->has('name'))
      ...
      <div class="col-md-6">
        ...
        <input type="text" id="price" name="price" class="form-control" placeholder="Price" pattern="\d+(\.\d{1,2})?" required value="{{old('price', $product->price) }}"/>
        @if ($errors->has('price'))
      ...
      <div class="col-md-6">
        ...
        {!! Form::select('category', [
         'Urns' => 'Urns', 
         'Burial Vaults' => 'Burial Vaults', 
         'Burial Mechandise' => 'Burial Mechandise', 
         'Casket' => 'Casket', 
         ], $product->category, ['class' => 'select2 form-select', 'id' => 'category', 'placeholder' => 'Select the appropriate information', 'required' => 'required']) !!}
      </div>

      <div class="col-md-6">
        <label class="form-label" for="multicol-first-name">Stock</label>
        <input type="number" id="stock" name="stock" class="form-control" placeholder="Stock" required value="{{old('stock', $product->stock) }}"/>
        @if ($errors->has('stock'))
      ...
      <div class="col-md-6">
        ...
        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="Description" name="description" required >{{old('description', $product->description) }}</textarea>
